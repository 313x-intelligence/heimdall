...
def cloud_trail_logs_not_public(credentials):
    ...
    trailBuckets = cloudTrailClient.describe_trails(
        trailNameList=core.get_trails_ARNs(credentials)
    )["trailList"]
    notExposed = []
    exposed = []
    if trailBuckets:
...
def ebs_not_public(credentials):
    ...
    for snapshot in ebsSnapshots:
        if snapshot is not None:
            snapshotAttribute = core.get_ec2_snapshot_attribute(
                snapshot, "createVolumePermission", credentials
            )
...
def rds_snapshot_is_public(snapshot, credentials):
    ...
    snapshotAttributes = cli.describe_db_snapshot_attributes(
        DBSnapshotIdentifier=snapshotIdentifier
    )
    for attribute in snapshotAttributes["DBSnapshotAttributesResult"][
        "DBSnapshotAttributes"
    ]:
        if (
            "all" in attribute["AttributeValues"]
            and attribute["AttributeName"] == "restore"
        ):
            return True
        else:
            return False
...
def rds_cluster_snapshot_is_public(snapshot, credentials):
    ...
    for attribute in snapshotAttributes["DBClusterSnapshotAttributesResult"][
...
def exposed_kms_keys(credentials):
    ...
    if keys:
        ...
        policies = list(
            map(
                lambda x: core.get_client("kms", credentials).get_key_policy(
                    KeyId=x, PolicyName="default"
                ),
                keyIds,
            )
        )
        for policy in policies:
            ...
            permissivePolicies = list(
                filter(core.permissive_policy, policy["Statement"])
            )
            notPermissivePolicies = list(
                filter(lambda x: not core.permissive_policy(x), policy["Statement"])
            )
            ...
            try:
                ...
                for grant in response["Grants"]:
                    if (
                        ...
                        and "AllUsers" in grant["Grantee"]["URI"]
                    ):
                        exposed.appent(name)
                    else:
                        notExposed.append(name)
