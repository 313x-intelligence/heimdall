...
def cloudtrail_encrypted(credentials):
    ...
    trails = list(
        map(
            lambda x: {"TrailARN": x["TrailARN"], "KmsKeyId": x.get("KmsKeyId")}, trails
        )
    )
...
def s3_default_encryption_enabled(credentials):
    ...
    for name in bucketNames:
        try:
            encryption = get_client("s3", credentials).get_bucket_encryption(
                Bucket=name
...
def rds_encrypted(credentials):
    ...
    encryptedInstances = list(
        filter(lambda x: x["StorageEncrypted"] == True, rdsInstances)
    )
    encryptedInstances = [i["DBInstanceIdentifier"] for i in encryptedInstances]
    noCryptoInstances = list(
        filter(lambda x: x["StorageEncrypted"] == False, rdsInstances)
    )
