...
def get_session(credentials):
    awsAccessKeyId, awsSecretAccessKey = credentials.get(
        "awsAccessKeyId"
    ), credentials.get("awsSecretAccessKey")
...
def get_client(resource, credentials):
    return get_session(credentials).client(
        resource, region_name=credentials.get("awsRegion")
    )
...
def permissive_policy(statement):
    return (
        ...
        or statement["Principal"].get("AWS") == "*"
        or statement["Principal"].get("CanonicalUser") == "*"
    )
